...
```mariadb
Добро пожаловать в монитор MariaDB.  Команды заканчиваются символом ; или \g.
Ваш идентификатор подключения MariaDB: 14
Версия сервера: 10.3.22-MariaDB-0ubuntu0.19.10.1 Ubuntu 19.10
...
```mariadb
ERROR 1045 (28000): Доступ запрещен для пользователя 'dvwa'@'localhost' (с использованием пароля: YES)
```
...
```mariadb
ОШИБКА 1044 (42000): Доступ запрещен для пользователя 'dvwa'@'localhost' к базе данных 'dvwa'
```
...
```mariadb
Фатальная ошибка: Неуловленное mysqli_sql_exception: Соединение отказано в /var/sites/dvwa/non-secure/htdocs/dvwa/includes/dvwaPage.inc.php:535
```
...
```sh
● mariadb.service - сервер базы данных MariaDB 10.5.19
     Загружено: загружено (/lib/systemd/system/mariadb.service; включено; предустановка: включено)
     Активен: активен (работает) с четверга 14 марта 2024 г. 16:04:25 GMT; 1 неделя 5 дней назад
```
...
```mariadb
Ошибка базы данных #2054: Сервер запросил метод аутентификации, неизвестный клиенту.
```
...
```
[Примечание] [Точка входа]: Запущен скрипт точки входа для MariaDB Server 1:10.11.15+maria~ubu2204.
[Warn] [Entrypoint]: /sys/fs/cgroup///memory.pressure не доступен для записи, функциональность недоступна для MariaDB
```
